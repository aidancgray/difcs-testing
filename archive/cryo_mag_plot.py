...
def animate(i, xs, tmag, msin, mcos, mpos, dps):
    ...
    mag_cos = rdng[1]
    mag_pos = rdng[-1]
    mag_pos_offset = (mag_pos-start_mag_pos) * 1000
    ...
    try:
        ...
        # tmag.append(temp_dif)
        msin.append(mag_sin)
        mcos.append(mag_cos)
        mpos.append(mag_pos_offset)
        dps.append(pos_2_um)

        # Limit x and y lists to DATA_LIMIT items
        xs = xs[-DATA_LIMIT:]
        # tmag = tmag[-DATA_LIMIT:]
        msin = msin[-DATA_LIMIT:]
        mcos = mcos[-DATA_LIMIT:]
        mpos = mpos[-DATA_LIMIT:]
        dps = dps[-DATA_LIMIT:]
        ...
        # l_tmag, = ax1.plot(xs, tmag, marker=marker_fmt, markersize=ms_fmt, 
        ...
        l_mpos, = ax2.plot(xs, mpos, marker=marker_fmt, markersize=ms_fmt, 
                           linewidth=lw_fmt, color='red')
        l_dps,  = ax2.plot(xs, dps, marker=marker_fmt, markersize=ms_fmt, 
        ...
        ax2.annotate(f'mag: {float("{0:.3f}".format(mag_pos_offset))} um', xy=xy_pos_0, xycoords='axes fraction',
                     size=10, ha='left', va='top', c='red',
                     bbox=dict(boxstyle='round', fc='w'))
        ax2.annotate(f'ids: {float("{0:.3f}".format(pos_2_um))} um', xy=xy_pos_1, xycoords='axes fraction',
                     size=10, ha='left', va='top', c='blue',
                     bbox=dict(boxstyle='round', fc='w'))
        ...
        return l_msin, l_mcos, l_mpos, l_dps
...
if __name__ == "__main__":
    ...
    header = ['time', 'temp', 'mag_sin', 'mag_cos', 'mag_pos', 'ids']
    ...
    ani = animation.FuncAnimation(fig, 
                                  animate, 
                                  fargs=(xs, tmag, msin, mcos, mpos, dps), 
                                  interval=ANIM_INTER,

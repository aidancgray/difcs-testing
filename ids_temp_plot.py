...
def animate(i, t, t_htr, t_a, t_b, t_c, t_d, ids_x, ids_y, ids_z):
    global data_count
    ...
    ax1.annotate(f'x_ids: {"{0:.3f}".format(ids_x[-1])}', xy=xy_pos_0, xycoords='axes fraction',
                    size=10, ha='left', va='top', color='red', 
                    bbox=dict(boxstyle='round', fc='w'))
    ax1.annotate(f'y_ids: {"{0:.3f}".format(ids_y[-1])}', xy=xy_pos_1, xycoords='axes fraction',
                    size=10, ha='left', va='top', color='blue', 
                    bbox=dict(boxstyle='round', fc='w'))
    ax1.annotate(f'z_ids: {"{0:.3f}".format(ids_z[-1])}', xy=xy_pos_2, xycoords='axes fraction',
    ...
    return l_ids_x, l_ids_y, l_ids_z #l_t_htr, l_t_a, l_t_b, l_t_c, l_t_d]
...
if __name__ == "__main__":
    dataFile = f"{DATA_PATH}{dt.datetime.now().strftime('%d%m%Y_%H-%M-%S')}-ids-temp.csv"
    header = ['time',
              'temp_htr', 
              'temp_a', 
              'temp_b', 
    ...
    l_ids_x, = ax1.plot(t, ids_x, marker=marker_fmt, markersize=ms_fmt, linewidth=lw_fmt, color='red')
    l_ids_y, = ax1.plot(t, ids_y, marker=marker_fmt, markersize=ms_fmt, linewidth=lw_fmt, color='blue')
    l_ids_y, = ax1.plot(t, ids_z, marker=marker_fmt, markersize=ms_fmt, linewidth=lw_fmt, color='green')
    ...
    try:
        ...
        ani = animation.FuncAnimation(fig, 
                                      animate, 
                                      fargs=(t, t_htr, t_a, t_b, t_c, t_d, ids_x, ids_y, ids_z), 
                                      interval=ANIM_INTER,
